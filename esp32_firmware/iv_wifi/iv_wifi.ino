...
#define PRIME_DURATION_MS         2000

// Serial Monitor status delay
#define SERIAL_PRINT_DELAY_MS     1000
...
void handleCommand(const String& raw) {
  ...
  if (cmd.startsWith("PARALLEL ")) {
    ...
    if (seqStepCount >= 3) {
      webSocket.sendTXT("ERROR: max 3 steps");
      return;
    }
    ...
    if (seqStepCount == 0) {
      webSocket.sendTXT("ERROR: no steps configured");
      return;
    }

    if (seqRunning) {
      webSocket.sendTXT("ERROR: sequence already running");
      return;
    }
...
void loop() {
  ...
  // Print status every 1000 ms without blocking the code
  if (millis() - lastSerialPrintMs >= SERIAL_PRINT_DELAY_MS) {
    lastSerialPrintMs = millis();

    Serial.print("[STATUS] ESP32 running | WiFi: ");
    ...
    Serial.print(" | Seq: ");
    if (seqRunning) {
      Serial.print(seqPaused ? "PAUSED" : "RUNNING");
    } else {
